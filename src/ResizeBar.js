...
obj.setS = function setS({ top }) {
  this.refs.el.style.bottom =
    -(top - this.initPos.top + this.initRelactivePos.bottom) + 'px';
};
obj.setE = function setE({ left }) {
  this.refs.el.style.right =
    -(left - this.initPos.left + this.initRelactivePos.right) + 'px';
};
...
obj.onMouseUp = function onMouseUp(evt) {
  ...
  let diff = this.normalize(evt.clientY - this.diffY, evt.clientX - this.diffX);
...
obj.render = function render() {
  const className = classNames('rdl-resize-bar', types[this.props.type]);
  return <div ref="el" className={className} onMouseDown={this.onMouseDown} />;
};

const ResizeBar = createClass(obj);
