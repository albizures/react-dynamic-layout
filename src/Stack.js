...
obj.getInitialState = function getInitialState() {
  ...
  if (
    this.props.active > -1 &&
    this.props.active < this.props.components.length
  ) {
    return {
...
obj.getLayout = function getLayout(component, className) {
  ...
  return (
    <div className={'rdl-item-body ' + className} key={component.id}>
      <Layout
...
obj.getComponent = function getComponent(data, className) {
  ...
  return (
    <div className={'rdl-item-body ' + className} key={data.id}>
      <Component
        {...data.props}
        ...
        rdChangeProps={(id, props) =>
          store.dispatch(updateComponentProps(id, props))
        }
      />
    </div>
  );
  ...
  for (let index = 0; index < this.props.components.length; index++) {
    ...
    tabs.push(
      <div
        ...
        key={component.id}
        className={'rdl-tab ' + activeClass}
      >
        {component.name}
      </div>,
    );
    if (!(this.context.hiddenType === RENDER && !isActive)) {
      children.push(this.getChild(component, activeClass));
    }
  }
  return {
    tabs,
...
obj.render = function render() {
  ...
  return (
    <div className="rdl-stack">
      {this.props.tabs ? (
        <div className="rdl-tabs" style={{ height: tabHeight }}>
          {tabs}
        </div>
      ) : null}
